...
serve(async (req) => {
  ...
  try {
    const { imageData, mimeType, fileName }: ImageResumeRequest = await req.json();
    ...
    // Comprehensive resume analysis prompt for images
    const prompt = `
You are an expert HR analyst and resume parser. Analyze this image which contains a resume and extract comprehensive information into a structured format. Be thorough and look for subtle details that might indicate exceptional talent, unique experiences, or potential concerns.

CRITICAL LINK EXTRACTION INSTRUCTIONS:
- Social media profiles and websites are often formatted as text without full URLs in resume images
...
Resume image content analysis:
"""
Please carefully read all text in the image and extract the information comprehensively.
"""
...
Focus on:
1. Extracting ALL relevant information comprehensively from the image
...
7. Aggressive link extraction and URL construction

Be analytical and thorough. Return only the JSON object, no additional text.
`;

    // Call Vertex AI with inline image data
    ...
    return new Response(
      JSON.stringify({ 
        ...
        processedResume,
        source: 'image',
        originalContent: `Image: ${fileName || 'resume-image'}`,
    ...
    console.error('Error in process-resume-image function:', error);
    
    return new Response(
      JSON.stringify({ 
        error: 'Failed to process resume image',
